...
void VkApp::submitFrame()
{
    ...
    submitInfo.pWaitSemaphores = &m_imageAvailableSemaphores[m_currentFrame];  // waited upon before execution
    submitInfo.signalSemaphoreCount = 1;
    submitInfo.pSignalSemaphores    = &m_renderFinishedSemaphores[m_currentFrame]; // signaled when execution finishes
    submitInfo.commandBufferCount = 1;
    submitInfo.pCommandBuffers = &m_currentCommandBuffer;
    VkResult result = vkQueueSubmit(m_queue, 1, &submitInfo, m_inFlightFences[m_currentFrame]);
    if (result != VK_SUCCESS)
        throw std::runtime_error( std::string("Failure to submit queue: ") + string_VkResult(result) );
    ...
    if (result != VK_SUCCESS)
        throw std::runtime_error( std::string("Failure to present queue: ") + string_VkResult(result) );
