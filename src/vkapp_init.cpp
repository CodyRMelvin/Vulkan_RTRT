...
void VkApp::createPhysicalDevice()
{
    ...
    for (auto physicalDevice : physicalDevices) 
    {
        ...
        for (int j = 0; j < extCount; ++j )
             extensionNames.emplace(extensionProperties[j].extensionName );

        bool elligible = true;
        for ( size_t j = 0; j < reqDeviceExtensions.size(); ++j )
        {
            if ( extensionNames.find( std::string( reqDeviceExtensions[j] ) ) == extensionNames.end() )
            {
                printf("%s not found\n", reqDeviceExtensions[j] );
                elligible = false;
                break;
            }
        }
...
void VkApp::createDevice()
{
    ...
    if ( vkCreateDevice(m_physicalDevice, &deviceCreateInfo, nullptr, &m_device) != VK_SUCCESS )
        throw std::runtime_error("Failure to create device");
...
void VkApp::createCommandPool()
{
    ...
    if ( vkCreateCommandPool(m_device, &poolCreateInfo, nullptr, &m_cmdPool) != VK_SUCCESS )
        throw std::runtime_error("Failure to create command pool");
...
void VkApp::getSurface()
{
    ...
    if ( glfwCreateWindowSurface(m_instance, app->GLFW_window, nullptr, &m_surface) != VK_SUCCESS )
        throw std::runtime_error("Failure to create window surface");
    VkResult result = vkGetPhysicalDeviceSurfaceSupportKHR(m_physicalDevice, m_graphicsQueueIndex,
                                                           m_surface, &isSupported);
    if (result != VK_SUCCESS)
        throw std::runtime_error("Failure to get physical deice surface support");
...
void VkApp::createSwapchain()
{
    ...
    ( 
          "Chosen surface:\nSurface Format: %s\nSurface color: %s\n"
        , string_VkFormat(surfaceFormat)
        , string_VkColorSpaceKHR(surfaceColor) 
    );
   ...
   // createInfo.minImageCount            = 3;
    ...
    if (result != VK_SUCCESS)
        throw std::runtime_error( std::string("Failure to create swapchain: ") + string_VkResult(result) );
    ...
    std::vector<VkImage> m_swapchainImages{m_imageCount};
    result = vkGetSwapchainImagesKHR( m_device, m_swapchain, &m_imageCount, m_swapchainImages.data() );
    if (result != VK_SUCCESS)
        throw std::runtime_error( std::string("Failure to get swapchain images: ") + string_VkResult(result) );
