                           ...
                           VkMemoryPropertyFlags properties)
{
    ...
    VkResult result = vkCreateBuffer(m_device, &bufferInfo, nullptr, &wrap.buffer);
    ...
    result = vkAllocateMemory(m_device, &allocInfo, nullptr, &wrap.memory);
    if ( result != VK_SUCCESS )
        throw std::runtime_error( std::string("Failed to allocate memory: ") + string_VkResult(result) );

    result = vkBindBufferMemory(m_device, wrap.buffer, wrap.memory, 0);
    if ( result != VK_SUCCESS )
        throw std::runtime_error( std::string("Failed to bind buffer memory: ") + string_VkResult(result) );
                                   ...
                                   VkBufferUsageFlags     usage)
{
    ...
    void* dest;
    VkResult result = vkMapMemory(m_device, staging.memory, 0, size, 0, &dest);
    if ( result != VK_SUCCESS )
        throw std::runtime_error( std::string("vkMapMemory in initBufferWrapFromData: ") + string_VkResult(result) );
