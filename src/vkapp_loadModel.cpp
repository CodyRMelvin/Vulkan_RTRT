...
void VkApp::loadModel()
{
    ...
    std::string modelFile = "../../src/models/San_Miguel/san-miguel.obj";
...
bool VkApp::loadModel(const std::string& filename, glm::mat4 transform)
{
    ...
    {
        Material& mat = meshdata.materials[ meshdata.matIndx[i] ];
        if ( glm::dot( mat.emission, mat.emission ) > 0.f )
        {
            Emitter emitter;
            emitter.v0 = meshdata.vertices[ meshdata.indices[ 3 * i ] ].pos;
