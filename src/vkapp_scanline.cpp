...
void VkApp::createScRenderPass()
{
    ...
    VkResult result = vkCreateRenderPass(m_device, &renderPassInfo, nullptr, &m_scRenderPass);
    if (result != VK_SUCCESS)
        throw std::runtime_error( std::string("Failure to create render pass: ") + string_VkResult(result) );
    ...
    info.layers          = 1;
    result = vkCreateFramebuffer(m_device, &info, nullptr, &m_scFramebuffer);
    if (result != VK_SUCCESS)
        throw std::runtime_error( std::string("Failure to create frame buffeframe bufferr: ") + string_VkResult(result) );

    
    // @@ Verify success of vkCreateRenderPass and vkCreateFramebuffer
    // @@ Destroy wioh vkDestroyRenderPass(m_device, m_scRenderPass, nullptr);
...
void VkApp::createScPipeline()
{
    ...
    VkResult result = vkCreatePipelineLayout(m_device, &createInfo, nullptr, &m_scPipelineLayout);
    if ( result != VK_SUCCESS )
    ...
    result = vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr,
                              &m_scPipeline);
    if ( result != VK_SUCCESS )
        throw std::runtime_error( std::string("Failure to create graphics pipeline: ") + string_VkResult(result) );
...
void VkApp::rasterize()
{
    ...
    vkCmdBeginRenderPass(m_currentCommandBuffer, &beginInfo, VK_SUBPASS_CONTENTS_INLINE);

    vkCmdBindPipeline(m_currentCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_scPipeline);
    ...
    for(const ObjInst& inst : m_objInst) {
        ...
        vkCmdPushConstants(m_currentCommandBuffer, m_scPipelineLayout,
                           VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                           sizeof(PushConstantRaster), &pcRaster);
        vkCmdBindVertexBuffers(m_currentCommandBuffer, 0, 1, &object.vertexBuffer.buffer, &offset);
        vkCmdBindIndexBuffer(m_currentCommandBuffer, object.indexBuffer.buffer, 0,
                             VK_INDEX_TYPE_UINT32);
        vkCmdDrawIndexed(m_currentCommandBuffer, object.nbIndices, 1, 0, 0, 0); }
...
void VkApp::updateCameraBuffer()
{
    ...
    vkCmdPipelineBarrier(m_currentCommandBuffer, uboUsageStages, VK_PIPELINE_STAGE_TRANSFER_BIT,
    ...
    vkCmdUpdateBuffer(m_currentCommandBuffer, m_matrixBuff.buffer, 0, sizeof(MatrixUniforms), &hostUBO);
    ...
    vkCmdPipelineBarrier(m_currentCommandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, uboUsageStages,
                         VK_DEPENDENCY_DEVICE_GROUP_BIT, 0,
